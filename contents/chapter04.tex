...
The analysis of this section reports ATT estimation results with the rigorously determined anticipation parameter (see~\nameref{sec:anticipation}) under the assumption of unconditional PTA.
...
Regarding the test of PTA in pre-treatment periods, we can observe that in all figures in \nameref{appendix_event_centered_trends}, the treated group's re-centered mean outcome trajectories highly overlap with those of the control group in the pre-treatment periods.
...
Lastly, in this section, we only report the aggregate event study plots over treatment-timing cohorts. For the exact estimated coefficients, please refer to~\nameref{complete_event_study_callaway}, and for event study plots specific to different treatment timing cohorts, please check the section:~\nameref{complete_group_time_att_dynamics}.
